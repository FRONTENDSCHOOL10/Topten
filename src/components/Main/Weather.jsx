...
function Weather() {
  ...
  const fetchHourlyWeatherData = async (lat, lon) => {
    ...
    try {
      ...
      if (response.data.response?.body?.items?.item) {
        ...
        const currentHour = currentTime.getHours() * 100 + (currentTime.getMinutes() >= 30 ? 100 : 0); 
        const currentDateStr = `${year}${month}${day}`;
  
        const filteredData = data.filter(
          (item) =>
            (item.category === 'TMP' || item.category === 'SKY' || item.category === 'PTY') &&
            (item.fcstDate > currentDateStr || (item.fcstDate === currentDateStr && parseInt(item.fcstTime, 10) >= currentHour))
        );
  ...
  const getWeatherIconSm = (condition) => {
    switch (condition) {
      ...
      case '눈':
        return { src: '/icon/icon-sm-snow2.png', alt: '눈' };
      case '소나기':
